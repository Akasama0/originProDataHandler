...
from dataHandler import ALL_PARAMETERS, DEFAULT_TARGET_COLUMNS, process_file, process_folder
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.input_label_var = tk.StringVar(value="Input CSV file:")
        self.output_label_var = tk.StringVar(value="Output .opju file:")
        ...
        tk.Entry(self, textvariable=self.csv_path, width=60).grid(row=1, column=1, **pad)
        ...
        tk.Entry(self, textvariable=self.opju_path, width=60).grid(row=2, column=1, **pad)
        ...
        self.run_button.grid(row=5, column=1, pady=12)

        # Log area
        self.log_widget = scrolledtext.ScrolledText(self, height=15, state="disabled")
        self.log_widget.grid(row=6, column=0, columnspan=3, sticky="nsew", padx=8, pady=8)
        ...
        if self.mode.get() == "single":
            ...
            if path:
                ...
                if not self.opju_path.get():
                    ...
                    self.opju_path.set(os.path.join(path, default_name))
            ...
            try:
                if batch:
                    success_count, failure_count, saved = process_folder(
                        input_path, output_path, target_columns=target_columns, log=self.log
                    )
                    success = saved
                    if success and failure_count > 0:
                        warning = f"{failure_count} file(s) were skipped. See log for details."
                else:
                    success = process_file(input_path, output_path, target_columns=target_columns, log=self.log)
