...
print("Reading and cleaning raw measurement file...")
...
if not filtered_rows:
    print("Error: Could not find any 'DataName' or 'DataValue' rows in the file.")
    sys.exit()
...
# 5. Apply the headers to the data columns
    ...
    print(f"--> Available columns found in your file: {df_data.columns.tolist()}")
    ...
    # IMPORTANT: If the print statement above shows 'Vg' instead of 'VG', change it here!
    ...
    if missing_cols:
        print(f"--> ERROR: Still missing these columns: {missing_cols}")
        print("--> Please update the 'target_columns' list in the script to match the printed available columns.")
        sys.exit()
        
    df_extracted = df_data[target_columns].dropna()

except KeyError as e:
    print(f"Error: Could not find columns 'VG' or 'ID'. Check the exact spelling in your DataName row. {e}")
    sys.exit()
except Exception as e:
...
try:
    ...
    print("Pushing extracted data to Origin...")
    ...
    print(f"Saving project to {opju_file_path}...")
    op.save(opju_file_path)
    print("Extraction and conversion completed successfully!")

except Exception as e:
    print(f"An error occurred in Origin: {e}")
